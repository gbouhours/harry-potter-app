<template>
  <div>
    <div>
      <label class="block mb-2 font-bold text-left">Réalisateur du film</label>
...
export default {
  ...
  setup(props) {
    ...
    // Call the useFormValdiation() function inside the setup() function on the right and get the validateTitleField() function and errors variable by destructuring the returned object on the left.
    const { validateDirectorField, errors } = useFormValidation();
    const validateInput = () => {
    ...
    validateDirectorField("director", input.value);
    };
